...
class Retriever:
    ...
    async def retriever_init(
        ...
        elif self.backend == "ali":
            # Aliyun Bailian (DashScope) embeddings via HTTP API
            # No SDK object needed; store cfg for later HTTP calls
            model_name = self.cfg.get("model_name")
            api_key = self.cfg.get("api_key") or os.environ.get("RETRIEVER_API_KEY")
            base_url = self.cfg.get("base_url")

            default_multimodal_url = (
                "https://dashscope.aliyuncs.com/api/v1/services/embeddings/multimodal-embedding/multimodal-embedding"
            )

            # If user passes compatible-mode URL, override to official multimodal endpoint
            if base_url is None or "compatible-mode" in str(base_url):
                if base_url and "compatible-mode" in str(base_url):
                    app.logger.warning(
                        "[ali] base_url points to compatible-mode; overriding to multimodal embedding endpoint."
                    )
                base_url = default_multimodal_url

            if not model_name:
                err_msg = "[ali] model_name is required"
                app.logger.error(err_msg)
                raise ValueError(err_msg)
            if not api_key:
                err_msg = "[ali] api_key is required"
                app.logger.error(err_msg)
                raise ValueError(err_msg)

            self.model_name = model_name
            self.api_key = api_key
            self.ali_api_url = base_url
            app.logger.info("[ali] Aliyun Bailian client configured (HTTP)")
        ...
        if self.backend in ["infinity", "sentence_transformers", "openai", "ali"]:
    ...
    async def retriever_embed(
        ...
        elif self.backend in ["openai", "ali"]:
            embeddings: list = []
            
            if is_multimodal:
                # Multi-modal embeddings using Aliyun Bailian API
                ...
                api_key = self.cfg.get("api_key") if self.backend == "openai" else getattr(self, "api_key", None)
                headers = {
                    "Authorization": f"Bearer {api_key}",
                    "Content-Type": "application/json"
                }
                
                # qwen2.5-vl-embedding only allows 1 image per request; send one by one.
                timeout = aiohttp.ClientTimeout(total=300)
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    with tqdm(
                        total=len(self.contents),
                        desc="[openai] Image Embedding:" if self.backend == "openai" else "[ali] Image Embedding:",
                        unit="item",
                    ) as pbar:
                        for start in range(0, len(self.contents), self.batch_size):
                            ...
                            for img_path in chunk:
                                try:
                                    ...
                                    err_msg = f"Failed to encode image {img_path}: {e}"
                                    app.logger.error(err_msg)
                                    raise RuntimeError(err_msg)
                                    ...
                                    async with session.post(
                                        ...
                                        if resp.status == 200:
                                            ...
                                            if "output" in result and "embeddings" in result["output"]:
                                                emb_list = result["output"]["embeddings"]
                                                if not emb_list:
                                                    raise RuntimeError("No embeddings returned for image request")
                                                embeddings.append(emb_list[0]["embedding"])
                                            else:
                                                err_msg = f"Unexpected response structure: {result}"
                                                app.logger.error(err_msg)
                                                raise RuntimeError(err_msg)
                                        else:
                                            err_msg = f"API request failed with status {resp.status}: {await resp.text()}"
                                            app.logger.error(err_msg)
                                            raise RuntimeError(err_msg)
                                except Exception as e:
                                    err_msg = f"[{self.backend}] Failed to embed images via Aliyun API: {e}"
                                    app.logger.error(err_msg)
                                    raise RuntimeError(err_msg)
                ...
                if self.backend == "openai":
                    with tqdm(
                        ...
                        for start in range(0, len(self.contents), self.batch_size):
                            ...
                            pbar.update(len(chunk))
                else:  # ali text embedding via HTTP
                    import aiohttp

                    api_url = self.ali_api_url
                    headers = {
                    ...
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        with tqdm(
                            ...
                            for start in range(0, len(self.contents), self.batch_size):
                                ...
                                payload = {
                                    "model": self.model_name,
                                    "input": {"contents": [{"text": t} for t in chunk]},
                                }
                                ...
                                try:
                                    async with session.post(
                                        ...
                                        if resp.status == 200:
                                            result = await resp.json()
                                            if "output" in result and "embeddings" in result["output"]:
                                                for emb_obj in result["output"]["embeddings"]:
                                    ...
                                    err_msg = f"[ali] Failed to embed text via Aliyun API: {e}"
                                    app.logger.error(err_msg)
                                    raise RuntimeError(err_msg)
                ...
                for i in tqdm(
                    ...
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            if "output" in result and "embeddings" in result["output"]:
                                for emb_obj in result["output"]["embeddings"]:
                                    query_embedding.append(emb_obj["embedding"])
                            else:
                                err_msg = f"Unexpected response structure: {result}"
                                app.logger.error(err_msg)
                                raise RuntimeError(err_msg)
